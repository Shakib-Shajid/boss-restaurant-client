...
const Login = () => {
    ...
    const navigate = useNavigate();         // all for navgate
    const location = useLocation();         // all for navgate

    const from = location.state?.from?.pathname || "/"; //use for navigate 1st step
    ...
    const handleLogin = event => {
        ...
        signIn(email, password)
            .then((result) => {
                ...
                navigate(from, { replace: true });      //use for navigate 2nd step
            })
            .catch((error) => {
